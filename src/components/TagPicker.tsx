    ...
    <div className="flex flex-col gap-2 mt-2 bg-white/5 p-2 rounded-lg border border-black/5">
      ...
      <div className="flex flex-wrap gap-1">
        {tags.map((t) => (
          <button
            key={t}
            className={`text-[9px] uppercase font-bold px-1.5 py-0.5 rounded transition-all ${selected.includes(t) ? "bg-(--accent) text-white" : "bg-black/10 text-(--text) opacity-60 hover:opacity-100"}`}
            onClick={() => {
